...
{block name="footer" literal=true}
    <script>
        ...
        $('#newPass').data({
            'v@rule': {r: true, minlen: 6, maxlen: 20},
            'v@message': {r: '请输入新密码', minlen: '密码至少是6个字符以上', maxlen: '密码过长，不可超过20个字符'}
        });
        $('#cfmPass').data({
            'v@rule': {r: true, eqto: '#newPass'},
            'v@message': {r: '请再次输入新密码', eqto: '两次输入的密码不一致'}
        });
    </script>
{/block}