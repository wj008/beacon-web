...
<head>
    ...
    {literal}
        <style type="text/css">
            ...
            .face {position: absolute;top: 8px;transform: rotate(90deg); color: #d00074;text-align: center; width: 42px; height: 42px; line-height: 35px; display: inline-block; font-size: 36px;}
...
<div id="box" class="box">
    <div class="msg"><b class="face">:(</b>
        <p>{$info.msg}</p></div>
    {if !empty($info.back)}
        <p class="auto-back">页面自动 <a id="back" href="{$info.back}">跳转</a> 等待时间： <b id="wait">{$info.timeout|default:3}</b></p>
    {else}
        <p class="auto-back">页面自动 <a id="back" id="back" href="javascript:;">关闭</a> 等待时间： <b id="wait">{$info.timeout|default:3}</b></p>
    {/if}
